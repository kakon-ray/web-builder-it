    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12 pb-4 d-flex justify-content-between">
                <h3 class="text-center ">Manage<span style="color:#4e73df;"> Client Review</span></h3>
                <a href="{{ route('dashboard.review.add')}}" type="button"
                class="btn btn-primary"><i class="fas fa-plus me-2"></i>Add Client Review</a>
            </div>
                    ...
                    <tbody>
                        @foreach ($allReview as $item)
                            <tr class="text-center">
                                ...
                                <td class="th-sm ">{{ $item->categorie == 'student_review' ? 'Student Review' : 'Client Review' }}</td>
                                <td class="th-sm ">{{ $item->review_star }}</td>
                                <td class="th-sm ">{{ $item->description }}</td>

                 
                                <td class="th-sm">
                                    <a href="{{ route('dashboard.review.update', ['id' => $item->id]) }}" type="button"
